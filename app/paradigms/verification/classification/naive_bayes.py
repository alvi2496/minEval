...
def cross_verification(data, n):
	count_resut_array, count_result = verifier.cross_verifier(naive_bayes.MultinomialNB(), processor.count_vectorizer(data), data['label'], n)
	tf_idf_result_array, tf_idf_result = verifier.cross_verifier(naive_bayes.MultinomialNB(), processor.tf_idf_vectorizer(data), data['label'], n)
