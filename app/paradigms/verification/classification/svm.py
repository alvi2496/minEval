...
def cross_verification(data, feature_vectors, n):
	count_resut_array, count_result = verifier.cross_verifier(svm.SVC(), \
		feature_vectors['count'], data['label'], n)
	tf_idf_result_array, tf_idf_result = verifier.cross_verifier(svm.SVC(), \
		feature_vectors['tf-idf'], data['label'], n)
	word_embedd_result_array, word_embedd_result = verifier.cross_verifier(svm.SVC(), \
